...
- **Генератор понижается в класс-итератор** (`GeneratorLowerer` в `preprocessTree`): узел
  `GeneratorDefinition` заменяется классом, реализующим `java.util.Iterator<T>`, а вызовы генератора —
  созданием объекта этого класса. Понижаются только две распознаваемые формы: тело из пролога
  и одного цикла с единственной выдачей, и тело из пролога и одних выдач. Всё остальное
  отвергается `UnloweredGeneratorFeature` с указанием причины — неверный перевод хуже отказа.
  Класс, реализующий `Iterator` и содержащий `hasNext()` и `next()`, разбирается обратно в
  `IteratorDefinition`. Пролог генератора при этом выполняется конструктором, а не при первом
  обходе: генератор в исходном языке ленив целиком, итератор — нет. Подробности и полный список
  отвергаемых форм — в
